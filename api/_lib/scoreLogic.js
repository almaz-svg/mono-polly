...
function smartFallbackScore(features, peerScore) {
  ...
  return { score, reason, source: 'fallback' };
}
...
function parseGrokJson(text) {
  ...
  const score = Math.round(Math.min(10, Math.max(1, Number(parsed.score) || 5)) * 10) / 10;
  const reason = typeof parsed.reason === 'string' && parsed.reason.trim()
    ? parsed.reason.trim()
    : 'Оценка от AI.';
  return { score, reason, source: 'grok' };
}

export async function scoreSubmission(features, peerScore, activeEventText, screenshotUrl, apiKey) {
  if (apiKey) {
